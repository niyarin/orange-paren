...
(import (scheme base) (scheme write) (scheme read) (scheme eval) (scheme process-context)
        (srfi 18) (srfi 106)
        (prefix (orange-paren repl) orepl/) (lib-simple-server))
    ...
    (let loop ()
             (let ((obj (read input-port)))
               (unless (eof-object? obj)
                  (let ((res (orepl/eval! obj repl-env)))
  ...
  (let loop ()
    ...
    (let ((input (read)))
      (if (eof-object? input)
        (exit 1)
        (begin
          (display (orepl/eval! input repl-env))(newline)
          (flush-output-port)
          (loop))))))
...
(let* ((repl-env (orepl/make-default-env))
